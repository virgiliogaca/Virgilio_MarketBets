...
import {
  ...
  require_object_is,
  require_object_keys
} from "./chunk-3FH2PCVP.js";
import {
  require_callBound,
  require_call_bind,
  require_is_arguments,
  require_shams
} from "./chunk-MXD3AEKK.js";
import {
  __commonJS,
  __export,
  __toModule,
    ...
    if (cell = cells[iCell]) {
      ...
      while (iHalfedge < nHalfedges) {
        ...
        if (Math.abs(endX - startX) > epsilon || Math.abs(endY - startY) > epsilon) {
          halfedges.splice(iHalfedge, 0, edges.push(createBorderEdge(site, end, Math.abs(endX - x05) < epsilon && y12 - endY > epsilon ? [x05, Math.abs(startX - x05) < epsilon ? startY : y12] : Math.abs(endY - y12) < epsilon && x12 - endX > epsilon ? [Math.abs(startY - y12) < epsilon ? startX : x12, y12] : Math.abs(endX - x12) < epsilon && endY - y05 > epsilon ? [x12, Math.abs(startX - x12) < epsilon ? startY : y05] : Math.abs(endY - y05) < epsilon && endX - x05 > epsilon ? [Math.abs(startY - y05) < epsilon ? startX : x05, y05] : null)) - 1);
          ++nHalfedges;
        }
      }
      if (nHalfedges)
...
// node_modules/react-vis/es/animation.js
var import_react_motion = __toModule(require_react_motion());
...
Bundle.prototype = {
  ...
  lineEnd: function() {
    ...
    if (j > 0) {
      ...
      while (++i <= j) {
        t = i / j;
        this._basis.point(this._beta * x4[i] + (1 - this._beta) * (x05 + t * dx), this._beta * y4[i] + (1 - this._beta) * (y05 + t * dy));
      }
    }
